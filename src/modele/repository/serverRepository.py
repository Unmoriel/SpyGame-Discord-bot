...
async def update_main_channel(id_server: str, main_channel: int):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE SERVERSDISCORD SET main_channel=%s WHERE id_server=%s",
        (main_channel, id_server)
    )
    conn.commit()
    cursor.close()
    conn.close()
...
async def update_recap_channel(id_server: str, recap_channel: int):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE SERVERSDISCORD SET recap_channel=%s WHERE id_server=%s",
        (recap_channel, id_server)
    )
    conn.commit()
    cursor.close()
    conn.close()
